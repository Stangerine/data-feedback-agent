...
归因类型与 7 个分析维度一一对应，外加"类间混淆"和"其他"两个跨维度类型：

| 归因类型 | 对应维度 | 含义 | 回流建议 |
|----------|----------|------|----------|
| 光照问题 | lighting | 光照条件（过暗/过亮/逆光）导致误报漏报 | 建议回流 |
| 视角问题 | viewpoint | 拍摄角度（仰角/俯拍/侧面）差异导致 | 建议回流 |
| 清晰度问题 | blur | 运动模糊/失焦/低分辨率导致 | 建议回流 |
| 天气问题 | weather | 雨雪雾等特殊天气条件导致 | 视频率而定 |
| 时段问题 | timeOfDay | 夜间/黄昏等时段差异导致 | 视频率而定 |
| 环境问题 | environment | 施工场景/城市街道/室内等环境差异 | 建议回流 |
| 类别偏差 | class | 训练集该类样本不足，分布不均 | **强烈建议回流** |
| 类间混淆 | — | 不同类别外观相似导致混淆 | 建议回流 |
| 其他 | — | 上述维度外的其他原因 | 需人工复核 |
